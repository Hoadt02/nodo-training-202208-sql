...
INSERT INTO DOTATHOA_SACH(ID, MA_SACH, TEN_SACH, ID_NXB, ID_TACGIA, CHU_DE, NAM_XUAT_BAN, MO_TA, SO_LUONG_CON_LAI, SO_LUONG_DANG_MUON, TONG_SO_SACH)
VALUES (SACH_SEQ_HOADT.nextval,'3008','XYZ',2,3,'ABC','03-DEC-2013','SS',500,4,504);
INSERT INTO DOTATHOA_SACH(ID, MA_SACH, TEN_SACH, ID_NXB, ID_TACGIA, CHU_DE, NAM_XUAT_BAN, MO_TA, SO_LUONG_CON_LAI, SO_LUONG_DANG_MUON, TONG_SO_SACH)
VALUES (SACH_SEQ_HOADT.nextval,'3009','CHUYẾN ĐI',5,2,'ABC','03-DEC-2013','SS',500,0,500);
...
INSERT INTO DOTATHOA_BANDOC(ID, MA_BANDOC, TEN_BANDOC, SO_DIEN_THOAI, DIA_CHI, NGAY_SINH, NGAY_TAO_TAI_KHOAN, HANG)
VALUES (BANDOC_SEQ_HOADT.nextval,4006,'NGUYỄN VIẾT HIÊN','0947656387','BẮC NINH','21-MAY-2002','05-JAN-2022',2);
...
SELECT *
FROM (
         SELECT MA_SACH, TEN_SACH, SUM(SO_LUONG_DANG_MUON) AS SL_DUOC_MUON
         FROM DOTATHOA_SACH S JOIN DOTATHOA_MUONSACH M ON S.ID = M.ID_SACH
                              JOIN DOTATHOA_BANDOC B ON M.ID_BANDOC = B.ID
         WHERE ((TO_CHAR(SYSDATE,'YYYY') - TO_CHAR(NGAY_SINH,'YYYY')) BETWEEN 18 AND 25) AND
               NGAY_GIO_MUON >= '01-JAN-2022'
         GROUP BY MA_SACH, TEN_SACH
         ORDER BY SL_DUOC_MUON DESC
     )
WHERE ROWNUM < 6;
...
FROM  DOTATHOA_BANDOC B LEFT OUTER JOIN DOTATHOA_MUONSACH M ON B.ID = M.ID_BANDOC
                       LEFT OUTER JOIN DOTATHOA_SACH S ON S.ID = M.ID_SACH;
